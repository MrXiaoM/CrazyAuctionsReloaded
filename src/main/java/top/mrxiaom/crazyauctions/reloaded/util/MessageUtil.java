...
public enum MessageUtil {
    ...
    public static void addMissingMessages() {
        FileManager.ProtectedConfiguration messages = FileManager.Files.MESSAGES.getFile();
    ...
    public String getMessage() {
        if (isList()) {
            if (exists()) {
                return PluginControl.color(convertList(FileManager.Files.MESSAGES.getFile().getStringList("Messages." + path)));
            } else {
                return PluginControl.color(convertList(getDefaultListMessage()));
            }
        } else {
            if (exists()) {
                return PluginControl.getPrefix(FileManager.Files.MESSAGES.getFile().getString("Messages." + path));
            } else {
    ...
    public String getMessage(HashMap<String, String> placeholders) {
        ...
        if (isList()) {
            if (exists()) {
                message = PluginControl.color(convertList(FileManager.Files.MESSAGES.getFile().getStringList("Messages." + path), placeholders));
            } else {
                message = PluginControl.color(convertList(getDefaultListMessage(), placeholders));
            }
        } else {
            if (exists()) {
                message = PluginControl.getPrefix(FileManager.Files.MESSAGES.getFile().getString("Messages." + path));
            } else {
    ...
    public String getMessageNoPrefix() {
        if (isList()) {
            ...
            if (exists()) {
                return PluginControl.color(FileManager.Files.MESSAGES.getFile().getString("Messages." + path));
            } else {
    ...
    public String getMessageNoPrefix(HashMap<String, String> placeholders) {
        ...
        if (isList()) {
            ...
            if (exists()) {
                message = PluginControl.color(FileManager.Files.MESSAGES.getFile().getString("Messages." + path));
            } else {
